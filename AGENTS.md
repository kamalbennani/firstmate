...
`bin/fm-brief.sh` and its help own scaffold syntax, generated variants, status protocol, and exact safety mechanics; `bin/fm-dod-lib.sh` owns each delivery mode's definition of done, which `bin/fm-spawn.sh` renders into every ship launch brief regardless of how the source brief was authored.
